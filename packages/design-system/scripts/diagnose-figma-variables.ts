...
export function diagnoseVariableFormat(variableDefs: Record<string, any>) {
  ...
  const targetVars = ['bg-subtle', 'bg-strong', 'Background/bg-subtle', 'Background/bg-strong'];
  ...
  if (!foundAny) {
    ...
    Object.keys(variableDefs).forEach(key => {
      if (key.toLowerCase().includes('bg-subtle') || 
        ...
        if (typeof variableDefs[key] === 'object' && variableDefs[key] !== null) {
          console.log(`     Structure:`, JSON.stringify(variableDefs[key], null, 2));
        }
      }
    });
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (input) {
    try {
      ...
      console.error("Usage: tsx diagnose-figma-variables.ts '{\"var\": \"value\"}'");
      process.exit(1);
    }
  } else {
    ...
    console.log('\n  3. Run this script:');
    console.log('     tsx scripts/diagnose-figma-variables.ts \'{"bg-subtle": {...}}\'');
    console.log('\n  Or import and use in code:');
    console.log('     import { diagnoseVariableFormat } from "./diagnose-figma-variables"');
